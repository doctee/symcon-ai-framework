...
| `289-combined-mqtt-position-saef-merge-and-standalone-publication.md` | Merges the reviewed SAEF candidate, publishes the exact five-file position-diagnostics delta to standalone main and remotely verifies complete-tree equality while preserving metadata and live gates. |
| `290-combined-mqtt-position-metadata-conformance.md` | Reproduces the official Validator runtime defect, then passes all 13 exact published metadata inputs through freshly downloaded official schemas and AJV 6.10.2 without accessing Symcon. |
| `291-combined-mqtt-position-disabled-symcon-rollout.md` | Installs the exact position-diagnostics commit through one supported disabled update, proves credential-free REST and logging compatibility, and transparently corrects a private null-state probe assumption. |
| `292-combined-mqtt-position-pilot-readiness.md` | Binds the combined format-3 harness to the installed commit and closes disabled baselines, passive token evidence and persistence acceptance while retaining a separate activation gate. |
| `293-combined-mqtt-position-activation-precondition-abort.md` | Records the single Gate L2 attempt stopping before mutation because the fresh token horizon was below threshold, with exact post-abort proof and no retry. |
| `294-combined-mqtt-position-retry-gate-readiness.md` | Stops a renewed activation gate before invoking the mutation candidate because the preceding read-only token horizon remained below threshold. |
| `295-combined-mqtt-position-pilot-activation.md` | Activates the combined receive-only transport and position pilot after a fresh passing token gate, then starts the 48-to-72-hour clock from two stable active format-3 baselines. |
| `296-combined-mqtt-position-pilot-deadline-status-review.md` | Closes the 72-hour observation as failed on transport episodes and position-counter monotonicity while preserving proof of four natural cycles and useful position reception. |
| `297-combined-mqtt-position-pilot-cleanup.md` | Closes the failed combined pilot through one Account ApplyChanges and proves immediate plus delayed disabled, credential-free stability with REST continuity. |
| `298-combined-mqtt-position-pilot-failure-analysis-and-stabilization-design.md` | Separates planned credential rotations from unexpected episodes, proves ephemeral cleanup as the position-counter reset cause and designs coordinate-free session accounting. |
...
| `313-automatic-pilot-closure-disabled-symcon-rollout.md` | Records the single supported disabled Symcon update to the automatic-closure commit, the corrected retention-aware probe and immediate plus delayed credential-free verification. |
| `314-automatic-pilot-closure-l2-readiness-abort.md` | Retains the fail-closed 2400-second preflight and supersedes its classification with the established 1200-second restart-free threshold while preserving the unused activation allowance. |
| `315-automatic-pilot-closure-l2-activation-safe-abort.md` | Records one restart-free activation reaching the accepted asynchronous reconnect state before an overstrict private synchronous check triggers verified mandatory cleanup, leaving any corrected retry separately gated. |
| `316-automatic-pilot-closure-l2-corrected-activation.md` | Records the separately authorized corrected activation, two-phase read-only proof of `ShadowActive` and the exact armed 72-hour automatic-closure deadline. |
| `317-automatic-pilot-closure-live-result.md` | Records automatic closure after the second transport episode, credential-first cleanup, retained MQTT and position evidence and delayed disabled-state verification. |
...
| `382-zone-mowing-recency-design-and-pilot-attempt.md` | Defines variable-first complete-zone mowing recency, a future bounded spatial raster and the cleaned first receive-only pilot attempt, with its historical status explicitly bounded by later steps. |
...
| `387-active-directional-map-marker-short-pilot.md` | Records the supervised 300-second receive-only proof of the active directional mower symbol and the automatic credential-free closure. |
| `388-continuous-receive-only-operating-decision.md` | Selects a private continuously monitored receive-only target with renewable bounded safety leases, finite recovery, visible freshness and unchanged REST authority while keeping implementation and activation closed. |
| `389-continuous-receive-only-implementation-design.md` | Freezes the backward-compatible mode policy, continuous registry, renewable lease, finite circuit recovery, operating variables, freshness presentation, implementation fileset and synthetic test matrix without productive code. |
...
| `413-legacy-root-scaffold-retirement.md` | Retires the obsolete root Navimow module tree after migrating its useful sanitized REST fixture contracts to the canonical distribution test. |
