...
adapter-managed immutable package bytes; instance objects, configurations and
...
full commit, expected instance and configuration identities, reference
identities, candidate package identity, protected migration-state root,
loopback RPC and fixed mutex names. No client or remote request may supply an
executable, module path, credential path or RPC destination.
...
7. Every expected MediaCarousel instance exists, is ready, has no pending
   change and matches its byte-exact configuration and complete reference
   identity.
...
apply with stale source, candidate, instance, configuration, reference, ACL or
policy identity fails before mutation.
...
2. repeat the source, candidate, instance, configuration and reference checks;
...
6. verify library and module identity, package identity, every instance,
   configuration, reference and ready status; and
...
5. prove the original package, module, instance, configuration, reference and
...
- configuration and reference drift before mutation;
