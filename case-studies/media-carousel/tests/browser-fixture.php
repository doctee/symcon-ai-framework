...
$fixtureScript = sprintf(
    '<script>window.fixtureRequests=[];window.requestAction=function(action,payload){if(action!=="LoadMedia"){return;}const request=JSON.parse(payload);window.fixtureRequests.push(request.index);window.setTimeout(function(){window.handleMessage(JSON.stringify({action:"media",configurationRevision:"browser-fixture-v2",requestID:request.requestID,index:request.index,source:%s[request.index],contentRevision:"fixture-"+request.index}));},request.index===4?650:60);};window.handleMessage(%s);</script>',
