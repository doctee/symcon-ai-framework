...
**Target:** `v0.2.0`
**Current decision:** LOCAL RELEASE PASS, CI PENDING
...
but the release must not be tagged until clean-checkout verification and CI
pass against the immutable release commit.
...
1. Run `composer validate --strict`, `make check` and generated-artifact drift
   checks from a clean checkout of the final revision.
2. Confirm that CI passes for that revision.
3. Create the annotated `v0.2.0` tag only after every pending gate is closed.
4. Verify that `.github/workflows/release.yml` extracts the v0.2.0 changelog
